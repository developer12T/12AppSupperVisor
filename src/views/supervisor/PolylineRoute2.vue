<template>
    ...
    <div class=" map-distance-overlay">
        <div class="flex justify-between">
            ...
            <div> จำนวนที่เช็คอินทั้งหมด: {{ uniquePolylineData.length }} จุด</div>
        </div>
...
// ฟังก์ชันแปลง format วันที่ เช่น "YYYY-MM-DD" -> "YYYYMMDD"
const formatDateToYMD = (dateStr) => {
    if (!dateStr) return '';
    return dateStr.replace(/-/g, '');
}

// ฟังก์ชันแปลง "YYYYMMDD" เป็น Date object เพื่อเปรียบเทียบ
const parseYMDDate = (ymdStr) => {
    if (!ymdStr || ymdStr.length !== 8) return null;
    const year = parseInt(ymdStr.substring(0, 4));
    const month = parseInt(ymdStr.substring(4, 6));
    const day = parseInt(ymdStr.substring(6, 8));
...
const uniquePolylineData = computed(() => {
    ...
    const startDateObj = filterByDate ? parseYMDDate(formatDateToYMD(startDate.value)) : null;
    const endDateObj = filterByDate ? parseYMDDate(formatDateToYMD(endDate.value)) : null;
    
    routeStore.polyline.forEach(item => {
        // ถ้าเลือกช่วงวันที่ให้ตรวจสอบ
        if (filterByDate && item.date) {
            const itemDateStr = item.date.replace(/\//g, '-'); // แปลง "DD/MM/YYYY" -> "DD-MM-YYYY"
            const [day, month, year] = itemDateStr.split('-');
            const itemDate = new Date(year, month - 1, day);
            
            // ถ้านอกช่วงวันที่ ให้ข้าม
...
function renderMap() {
    ...
    // เติม marker ใหม่จาก uniquePolylineData
    const latlngs = [];
    uniquePolylineData.value.forEach((item, idx) => {
        const icon = L.icon({
        ...
        const ll = [item.location[1], item.location[0]];
    ...
    // คำนวณระยะทาง
    let sum = 0;
    for (let i = 1; i < uniquePolylineData.value.length; i++) {
        sum += getDistance(
            uniquePolylineData.value[i - 1].location[1], uniquePolylineData.value[i - 1].location[0],
            uniquePolylineData.value[i].location[1], uniquePolylineData.value[i].location[0]
        );
    }
...
function gotoPoint(idx) {
    const item = uniquePolylineData.value[idx];
    if (!item) return;
