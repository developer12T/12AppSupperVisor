      ...
      <table class="table w-full min-w-[600px]">
        ...
        <tbody>
          <tr
            ...
            v-for="(item, idx) in routeStore.polyline"
            :key="item.storeId"
          >
            <td>{{ idx + 1 }}</td>
            <td>{{ item.storeId }}</td>
        ...
        >
          <tr class="bg-gray-300 font-bold">
            <td colspan="2" class="border p-2 text-left">
              รวมซื้อ {{ totalSale }}
            </td>
            <td colspan="2" class="border p-2 text-left">
              รวมไม่ซื้อ {{ totalNotSale }}
            </td>
            <td colspan="2" class="border p-2 text-left">
              รวมทั้งหมด {{ total }}
            </td>
...
function renderMap() {
  ...
  points.value.forEach((item, idx) => {
    ...
    const m = L.marker(ll, { icon }).bindPopup(
      `<b>StoreID:</b> ${item.storeId}<br><b>Route:</b> ${item.route}<br><b>Date:</b> ${item.date}`,
    );
...
async function onMonthChange() {
  ...
  try {
    ...
    await routeStore.getPolyLineCredit(
      period,
      route.params.area,
      startStr,
      endStr,
